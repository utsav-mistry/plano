            ...
            <div className="flex flex-col items-end gap-2">
               ...
               <div className="flex flex-col items-end mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-4 text-xs font-semibold">
                     ...
                     <span className="text-text-primary">{new Date(invoice.issueDate).toLocaleDateString()}</span>
               ...
               <tbody className="divide-y divide-gray-100">
                  {invoice.items && invoice.items.map((item: any, idx: number) => (
                    <tr key={idx} className="font-sans">
                       ...
                       <td className="py-4 px-2 text-sm font-mono font-bold text-text-primary text-right">{formatCurrency(item.amount, invoice.currency)}</td>
                    </tr>
                  ))}
               </tbody>
            </table>
         ...
         <div className="px-12 py-8 bg-zinc-50 border-t border-border flex justify-end">
            <div className="w-full max-w-xs flex flex-col gap-3">
               <div className="flex justify-between items-center text-xs font-bold uppercase tracking-widest text-gray-400">
                  <span>Subtotal:</span>
                  <span className="text-text-primary font-mono">{formatCurrency(invoice.subtotal, invoice.currency)}</span>
               </div>
               <div className="flex justify-between items-center text-xs font-bold uppercase tracking-widest text-gray-400">
                  <span>Tax:</span>
                  <span className="text-text-primary font-mono">{formatCurrency(invoice.taxAmount, invoice.currency)}</span>
               </div>
               ...
               <div className="flex justify-between items-center">
                  ...
                  <span className="text-3xl font-serif font-bold text-plano-900">{formatCurrency(invoice.totalAmount, invoice.currency)}</span>
               </div>
            </div>
         </div>
