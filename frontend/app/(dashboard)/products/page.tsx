      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map((product) => (
            <div key={product._id} className="bg-bg-surface border border-sidebar-hover rounded-card p-5 group hover:shadow-md transition-all flex flex-col gap-4">
              ...
              <div className="mt-4 pt-4 border-t border-sidebar-hover flex items-center justify-between">
                 <Link href={`/products/${product._id}/edit`} className="text-xs font-bold text-text-secondary hover:text-plano-400 transition-all flex items-center gap-1 uppercase tracking-widest">
                    Edit <Edit2 size={12} />
                 </Link>
                 <button 
           ...
           <div className="overflow-x-auto">
            <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-sidebar-hover">
                    {filteredProducts.map((product) => (
                      <tr key={product._id} className="group hover:bg-gray-25 dark:hover:bg-white/5 transition-colors">
                        ...
                        <td className="py-4 px-6 text-right">
                            <div className="flex items-center justify-end gap-2">
                                <Link href={`/products/${product._id}/edit`} className="p-1.5 rounded-btn hover:bg-sidebar-hover text-gray-400 hover:text-plano-400 transition-all">
                                  <Edit2 size={16} />
                                </Link>
                                <button 
                                  onClick={() => handleDelete(product._id)}
