...
export default function TaxesManagementPage() {
   ...
   async function handleSubmit(e: React.FormEvent) {
      ...
      try {
         ...
         const res = editingTaxId
            ? await api.taxes.update(editingTaxId, payload)
            : await api.taxes.create(payload);

         if (res.success) {
            success(editingTaxId ? 'Tax rule updated!' : 'Tax rule created!', 'Configuration saved successfully.');
            setShowDrawer(false);
         ...
         toastError(editingTaxId ? 'Failed to update tax' : 'Failed to create tax', err.message);
      } finally {
   ...
   function openEditDrawer(tax: any) {
               ...
               <form onSubmit={handleSubmit} className="w-full max-w-md bg-bg-surface dark:bg-bg-surface h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 border-l border-border dark:border-sidebar-hover">
                  <div className="p-6 border-b border-border dark:border-sidebar-hover flex items-center justify-between">
                     <div className="flex flex-col">
                        <h2 className="text-2xl font-serif font-bold text-text-primary leading-tight">{editingTaxId ? 'Edit Tax Rule' : 'Add Tax Rule'}</h2>
                        <span className="text-[10px] uppercase font-bold text-gray-400 tracking-widest mt-1">Fiscal Configuration</span>
                     </div>
                     <button type="button" onClick={() => { setShowDrawer(false); setEditingTaxId(null); }} className="p-2.5 rounded-full hover:bg-sidebar-hover text-gray-400 transition-colors">
                        <X size={20} />
                     </button>
                  </div>
                  ...
                  <div className="p-6 border-t border-border dark:border-sidebar-hover flex items-center justify-between gap-4 bg-bg-surface">
                     <button type="button" onClick={() => { setShowDrawer(false); setEditingTaxId(null); }} className="flex-1 h-12 rounded-xl border border-border dark:border-sidebar-hover bg-bg-page text-[11px] font-bold uppercase tracking-widest text-text-secondary hover:bg-sidebar-hover transition-all shadow-sm">
                        Discard
                     </button>
                     <button
                     ...
                     >
                        ...
                        {isSubmitting ? 'Processing...' : editingTaxId ? 'Update Tax Rule' : 'Create Tax Rule'}
                     </button>
                  </div>
               </form>
            </div>
         ...
         {deleteTarget && (
            <div className="fixed inset-0 z-[110] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
               <div className="w-full max-w-md rounded-card border border-sidebar-hover bg-bg-surface p-6 shadow-2xl">
                  <h3 className="text-xl font-serif font-bold text-text-primary">Delete Tax Rule</h3>
