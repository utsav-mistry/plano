...
export default function InvoiceDetailPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        toastError('Not Found', 'The requested invoice was not found.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        toastSuccess('Payment Recieved', 'Your invoice has been marked as paid successfully.');
        setIsPaying(false);
      }, 2000);
    } catch (err: any) {
      toastError('Payment Error', err.message || 'Unable to process payment sandbox.');
      setIsPaying(false);
    }
  };
  ...
  const handleDownload = async () => {
    if (!invoice) return;
    toastSuccess('Downloading...', 'Your invoice PDF is being generated.');
