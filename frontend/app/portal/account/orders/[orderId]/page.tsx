...
export default function OrderDetailPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (invsRes.success) setInvoices(invsRes.data);
      } catch (err) {
        console.error('Failed to load order details', err);
        toastError('Not Found', 'The requested order was not found.');
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleRenew = async () => {
    ...
    setIsRenewing(true);
    try {
      // Mock renew — usually creates a new subscription or extends current
