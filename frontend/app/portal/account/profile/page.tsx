...
export default function ProfilePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!user?._id) return;
      // Using form data directly for dynamic updates
      const res = await api.users.update(user._id, form);
      if (res.success) {
        toastSuccess('Profile Updated', 'Your details have been saved successfully.');
        // Optionally trigger a silent verifyAuth if available to refresh global user state
      }
    } catch (err: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-4 h-[calc(100vh-64px)] overflow-hidden flex flex-col">
       {/* Header */}
       ...
       <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 min-h-0">
          ...
          <div className="lg:col-span-3 h-full overflow-hidden flex flex-col">
             <motion.form 
               ...
               className="bg-white rounded-[2rem] border border-plano-100 p-6 md:p-8 shadow-xl shadow-plano-600/5 relative overflow-hidden flex flex-col h-fit"
             >
                {/* Aesthetic Sparkle */}
