...
export default function MyQuotesPage() {
  const [quotes, setQuotes] = useState<Subscription[]>([]);
    ...
    async function fetchQuotes() {
      try {
        ...
        if (res.success) {
          ...
          const all = Array.isArray(data) ? data : (data?.subscriptions || []);
          // In Section 10, a quotation is a draft state before confirmation
          setQuotes(all.filter((s: any) => s.status.toLowerCase() === 'quotation' || s.status.toLowerCase() === 'draft'));
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const filtered = quotes.filter(q => 
    ...
    (typeof q.productId === 'object' && q.productId.name.toLowerCase().includes(search.toLowerCase()))
  );
                ...
                <div className="block bg-white rounded-3xl border border-plano-50 p-6 hover:border-plano-600 hover:shadow-2xl hover:shadow-plano-600/5 transition-all group overflow-hidden relative">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center relative z-10">
                      <div className="md:col-span-4 flex items-center gap-6">
                         ...
                         <div>
                            <span className="text-[10px] font-bold text-plano-600 uppercase tracking-widest block mb-1">Quote Template #</span>
                            <h4 className="text-lg font-bold text-plano-900 uppercase tracking-tight">{quote.id}</h4>
                         </div>
                      </div>

                      <div className="md:col-span-3">
                         <div className="flex flex-col">
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1 italic">Service Package</span>
                            <span className="text-sm font-bold text-plano-900 uppercase">{typeof quote.productId === 'object' ? quote.productId.name : 'Custom Solution'}</span>
                         </div>
                      </div>
                      ...
                      <div className="md:col-span-2">
                         <div className="flex flex-col">
                            ...
                            <span className="text-xl font-bold text-plano-900 tabular-nums">₹{quote.grandTotal.toLocaleString()}</span>
                         </div>
                      </div>

                      <div className="md:col-span-3 flex justify-end gap-3">
                         <button className="h-12 px-6 rounded-xl border border-plano-100 text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-plano-600 hover:border-plano-600 transition-all">
                            View PDF
                         </button>
                         <button className="h-12 px-6 rounded-xl bg-plano-600 text-white text-[10px] font-bold uppercase tracking-widest hover:bg-plano-900 transition-all shadow-lg shadow-plano-600/20 flex items-center gap-2">
                            Accept Quote
                            <ArrowRight size={14} />
                         </button>
                      </div>
