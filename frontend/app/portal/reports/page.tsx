...
export default function ReportsPage() {
  ...
  const totalPaid = payments.filter(p => p.status === 'success' || p.status === 'completed').reduce((acc, p) => acc + p.amount, 0);
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-gray-25/50">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 py-6 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* Status Distribution */}
          <div className="lg:col-span-8 bg-white rounded-[2.5rem] border border-plano-50 shadow-sm p-6 sm:p-8 flex flex-col overflow-hidden">
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-[2rem] border border-plano-50 p-6 flex flex-col overflow-hidden shadow-sm">
                ...
                {payments.slice(0, 5).map((p, i) => (
                  <div key={p.id} className="flex items-center gap-3 group">
                    ...
                    <div className="flex-grow min-w-0">
                      ...
                      <p className="text-[9px] font-medium text-gray-400 uppercase tracking-wider">{new Date((p as any).processedAt || p.createdAt).toLocaleDateString()}</p>
                    </div>
